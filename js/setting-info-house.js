...
function getPersonInfo(){
    ...
    $.get(getApiUrl+'/dormitory/statistics/range?access_token='+sessionStorage.getItem('access_token'),function(data){
        var str = '';
        var name = '';
        if(data.data){
            $.each(data.data,function(m,n){
                name += n.dormitory_name;
                $('.info-range label').html(name);
                str += '<p>'+ n.dormitory_name+'<span>'+ n.bedroom_name+'</span></p>';
            });
        }
        $('.dorm-list').html(str);
    });
