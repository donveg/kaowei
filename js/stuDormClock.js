...
$('.after-w-icon').parent().click(function(){
    ...
    if(year == globalYear && currentMonth >= globalMonth){
        ...
        currentWeek += 1;
        if(currentWeek == getTotalWeek(year,currentMonth))
        {
            $('.week-list button .after-w-icon').parent().attr('disabled','disabled');
        }
    }else{
...
function getTotalData2(val1,val2,val3){
  ...
  //  $.each(listData,function(x,y){
        ...
        $('.shi').html(listData.actualSchedule+'天');
    //  });
       ...
       // $('.kuang-box,.chi-box,.zao-box').show();
        $.each(listData.list,function(m,n){
            if(n.name=='正常'){
                ...
                $.each(n.data,function(a,b){
                    var mm = parseInt((new Date(b.updated_at*1000)).getMonth())+1;
                    var fen = (new Date(b.updated_at*1000)).getHours()+':'+(new Date(b.updated_at*1000)).getMinutes()+':'+(new Date(b.updated_at*1000)).getSeconds();
                    var ddd = (new Date(b.updated_at*1000)).getFullYear()+'-'+mm+'-'+(new Date(b.updated_at*1000)).getDate()+' '+fen;
                ...
                $.each(n.data,function(a,b){
                    ...
                    var fen = (new Date(b.updated_at*1000)).getHours()+':'+(new Date(b.updated_at*1000)).getMinutes()+':'+(new Date(b.updated_at*1000)).getSeconds();
                    var ddd = (new Date(b.updated_at*1000)).getFullYear()+'-'+mm+'-'+(new Date(b.updated_at*1000)).getDate()+' '+fen;
                    que +='<div><span>'+ddd+'</span></div>';
                });
                $('.single-zao-box').html(que);
                ...
                $.each(n.data,function(a,b){
                    ...
                    var fen = (new Date(b.updated_at*1000)).getHours()+':'+(new Date(b.updated_at*1000)).getMinutes()+':'+(new Date(b.updated_at*1000)).getSeconds();
                    var ddd = (new Date(b.updated_at*1000)).getFullYear()+'-'+mm+'-'+(new Date(b.updated_at*1000)).getDate()+' '+fen;
                    wei +=' <div><span>'+ddd+'</span></div>';
                });
                $('.single-wei-box').html(wei);
                ...
                $.each(n.data,function(a,b){
                    ...
                    var fen = (new Date(b.updated_at*1000)).getHours()+':'+(new Date(b.updated_at*1000)).getMinutes()+':'+(new Date(b.updated_at*1000)).getSeconds();
                    var ddd = (new Date(b.updated_at*1000)).getFullYear()+'-'+mm+'-'+(new Date(b.updated_at*1000)).getDate()+' '+fen;;
                   yi +='<div><span>'+ddd+'</span></div>';
                });
                $('.single-yi-box').html(yi);

            }
        });
    }else{
        $('.kuang-box,.chi-box,.zao-box,.wei-box,.yi-box').hide();
    }
}
//时间选择插件 年-月-日
...
function singleJump(num){
    switch(num){
        case 1:
            $('.tab-change-list,.week-list,.month-list,.term-list,.single-chi-box,.single-zao-box,.single-wei-box,.single-yi-box').hide();
            $('.single-chi-box').show();
            document.title = '晚归';
            break;
        case 2:
            $('.tab-change-list,.week-list,.month-list,.term-list,.single-zao-box,.kuang-list-box,.single-wei-box,.single-yi-box').hide();
            $('.kuang-list-box').show();
            document.title = '早出';
            break;
        case 3:
            $('.tab-change-list,.week-list,.month-list,.term-list,.kuang-list-box,.single-chi-box,.single-wei-box,.single-yi-box').hide();
            $('.single-zao-box').show();
            document.title = '正常';
            break;
        case 4:
            $('.tab-change-list,.week-list,.month-list,.term-list,.kuang-list-box,.single-chi-box,.single-zao-box,.single-yi-box').hide();
            $('.single-wei-box').show();
            document.title = '未归';
            break;
        case 5:
            $('.tab-change-list,.week-list,.month-list,.term-list,.kuang-list-box,.single-chi-box,.single-zao-box,.single-wei-box').hide();
            $('.single-yi-box').show();
            document.title = '异常进入';
            break;
        default :
