...
flagJson = new Array;
flagBg = [];
globalType = 1;//1为白天 0为黑夜
globalTotalWay = 1; //默认日统计
(function(){

    $.ajaxSetup({
        async: false
    });
    $.get(getApiUrl+'/dormitory/statistics?access_token='+sessionStorage.getItem('access_token'),function(data){
        var str = '';
        if(data.code == 1 || data.code == 14){
            // showError(1,'您的账号暂未开通考勤、排课服务。 请联系本校教师或拨打服务热线','');return;
            $('.maskk,.no-access').show();
            $('.change-menu,.teach-my-schedule').hide();return;
        }
        if(data.code == 0)
        {
    ...
    $('.class-info-list').click(function(){
        ...
        $.get(getApiUrl+'/dormitory/statistics/grade?access_token='+sessionStorage.getItem('access_token'),function(data){
            if(data.code == 1){
                $('.maskk,.no-access').show();
            ...
            if(data.data.type == 1) //白天
            {
                if(data.data.list.abnormal.length >0){ //有异常
                    ...
                    $.each(data.data.list.abnormal,function(k,v){
                        ...
                        v.parents_phone ? ppCall = v.parents_phone : ppCall='空';
                        v.teacher_phone ? ttCall = v.teacher_phone : ttCall='空';

                        if(v.s_status == 2 ){ //晚归状态
                        ...
                        <p>此同学晚归寝室</p></div> <span class="abnormal-list-right" parents_phone="'+ppCall+'" teacher_phone="'+ttCall+'"><img src="images/big-call.png"  class="phoneToCall"></span></li>';

                        }else if(v.s_status == 5){ //未归状态

                            str +=' <li class="day-abnormal-li"><div class="abnormal-list-left"><img src="images/ic-yes.png"/></div>\
                        <div class="abnormal-list-middle"> <p><span>'+ v.username+'</span><i>'+ v.grade_name+' · '+ v.class_room_name+'</i><label>'+filterGender(v.gender)+'</label></p>\
                        <p>此同学异常进入寝室</p></div> <span class="abnormal-list-right" parents_phone="'+ppCall+'" teacher_phone="'+ttCall+'"><img src="images/big-call.png"  class="phoneToCall"></span></li>';

                        }else if(v.s_status == 1){
                            str +=' <li class="day-abnormal-li"><div class="abnormal-list-left"><img src="images/ic-yes.png"/></div>\
                        <div class="abnormal-list-middle"> <p><span>'+ v.username+'</span><i>'+ v.grade_name+' · '+ v.class_room_name+'</i><label>'+filterGender(v.gender)+'</label></p>\
                        <p>此同学在非在寝时间进入宿舍</p></div> <span class="abnormal-list-right" parents_phone="'+ppCall+'" teacher_phone="'+ttCall+'"><img src="images/big-call.png"  class="phoneToCall"></span></li>';
                        }
                    });
                ...
                if(data.data.list.normal.length >0){ //正常
                    ...
                    $.each(data.data.list.normal,function(k,v){
                        ...
                        <p class="noBody-css">正常离寝中</p></div> <span class="abnormal-list-right" parents_phone="'+ppCall+'" teacher_phone="'+ttCall+'"><img src="images/big-call.png"  class="phoneToCall"></span></li>';
                    });
            ...
            }else
            {
                if(data.data.list.abnormal.length >0){ //有异常
                    ...
                    $.each(data.data.list.abnormal,function(k,v){
                        ...
                        if(v.s_status == 3 ){ //早出状态
                            str +=' <li class="night-abnormal-li"><div class="abnormal-list-left"><img src="images/ic-no.png"/></div>\
                        <div class="abnormal-list-middle"> <p><span>'+ v.username+'</span><i>'+ v.grade_name+' · '+ v.class_room_name+'</i><label>'+filterGender(v.gender)+'</label></p>\
                        <p>此同学早出寝室</p></div> <span class="abnormal-list-right" parents_phone="'+ppCall+'" teacher_phone="'+ttCall+'"><img src="images/big-call.png"  class="phoneToCall"></span></li>';
                        }else if(v.s_status == 0){ //不在宿舍宿舍
                            str +=' <li class="night-abnormal-li"><div class="abnormal-list-left"><img src="images/ic-no.png"/></div>\
                        <div class="abnormal-list-middle"> <p><span>'+ v.username+'</span><i>'+ v.grade_name+' · '+ v.class_room_name+'</i><label>'+filterGender(v.gender)+'</label></p>\
                        <p>此同学未在寝室</p></div> <span class="abnormal-list-right" parents_phone="'+ppCall+'" teacher_phone="'+ttCall+'"><img src="images/big-call.png"  class="phoneToCall"></span></li>';
                        }else if(v.s_status == 4){
                            str +=' <li class="night-abnormal-li"><div class="abnormal-list-left"><img src="images/ic-no.png"/></div>\
                        <div class="abnormal-list-middle"> <p><span>'+ v.username+'</span><i>'+ v.grade_name+' · '+ v.class_room_name+'</i><label>'+filterGender(v.gender)+'</label></p>\
                        <p>此同学未归寝室</p></div> <span class="abnormal-list-right" parents_phone="'+ppCall+'" teacher_phone="'+ttCall+'"><img src="images/big-call.png"  class="phoneToCall"></span></li>';
                        }
                    });
                    $('.abnormal-list').html(str);
                }
                ...
                if(data.data.list.normal.length >0){ //正常
                    ...
                    $.each(data.data.list.normal,function(k,v){
                        ...
                        str +=' <li class="day-abnormal-li"><div class="abnormal-list-left"><img src="images/ic-yes.png"/></div>\
                        <div class="abnormal-list-middle"> <p><span>'+ v.username+'</span><i>'+ v.grade_name+' · '+ v.class_room_name+'</i><label>'+filterGender(v.gender)+'</label></p>\
                        <p>正常在寝</p></div> <span class="abnormal-list-right" parents_phone="'+ppCall+'" teacher_phone="'+ttCall+'"><img src="images/big-call.png" class="phoneToCall"></span></li>';
                    });
                    $('.normal-list').html(str);
                }
            }
...
function setInfo()
{
    ...
    $.get(getApiUrl+'/dormitory/statistics/list?access_token='+sessionStorage.getItem('access_token')+'&gid='+gid+'&cid='+cid+'&date='+da,function(data){
        if(data.code == 1 || data.code == 14){
           // showError(1,'您的账号暂未开通考勤、排课服务。 请联系本校教师或拨打服务热线','');return;
           $('.maskk,.no-access').show();
           $('.change-menu,.teach-my-schedule').hide();return;
...
function setInfo2(start,end)
{
    ...
    $.get(getApiUrl+'/dormitory/statistics/list?access_token='+sessionStorage.getItem('access_token')+'&gid='+gid+'&cid='+cid+'&start='+start+'&end='+end,function(data){
        if(data.code == 1 || data.code == 14){
            // showError(1,'您的账号暂未开通考勤、排课服务。 请联系本校教师或拨打服务热线','');return;
            $('.maskk,.no-access').show();
            $('.change-menu,.teach-my-schedule').hide();return;
...
function getClassData(){
    var gid = $('.grade option:selected').val();
    garname = gid;
    $.get(getApiUrl+'/dormitory/statistics/class-room?access_token='+sessionStorage.getItem('access_token')+'&id='+gid,function(data){
        if(data.code == 1){
            showError(1,'您的账号暂未开通考勤、排课服务。请联系本校教师或拨打服务热线','');return;
        }
