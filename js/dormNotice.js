(function(){
    ...
    $('.notice-list').on('click','li',function(){
        ...
        $.get(getApiUrl+'/dormitory/notice/edit?access_token='+sessionStorage.getItem('access_token')+'&id='+$(this).attr('data-id'),function(data){
            if(data.code == 0){
    ...
    $('.delete-notice').click(function(){
        if(confirm('确定要删除吗？'))
        {
            ...
            $.get(getApiUrl+'/dormitory/notice/delete?access_token='+sessionStorage.getItem('access_token')+'&id='+id,function(data){
                if(data.code == 0)
                {
                    window.location.href="dorm-notice.html";
                }else{
                    alert(data.message)
                }
            });
...
function addList(id,status) // 1代表新增  2代表编辑 保存
{
    ...
    $.post(getApiUrl+'/dormitory/notice/update?access_token='+sessionStorage.getItem('access_token'),data,function(data){
        if(data.code == 0)
        {
            window.location.href="dorm-notice.html";
        }else{
...
{
    var str = '';
    $.get(getApiUrl+'/dormitory/notice?access_token='+sessionStorage.getItem('access_token')+'&page='+page,function(data){
        ...
        if(state==true){
            ...
            setTimeout(function(){
                ...
                $.ajax({
                    ...
                    url:getApiUrl+'/dormitory/notice?access_token='+sessionStorage.getItem('access_token')+'&page='+$('.caseud').attr('data-nowPage'),
                    type:'GET',
                    cache:false,
                    dataType:"html",
