(function(){
    ...
    $('.term').click(function(){
        ...
        getTotalData2('',firstSTime,firstETime);
    });
    ...
    $('.semester button').click(function(){
        ...
        if($(this).find('i').hasClass('before-x-icon')){
            ...
            var ttt = globalTermKey -1;
            if(semesterArrKey >= ttt){
                semesterArrKey = 1;
...
$('.before-w-icon').parent().click(function(){
    //console.log('globalWeek='+globalWeek);
    ...
    if(year == globalYear && month ==  globalMonth && currentWeek < globalWeek ){
        $('.week-list button .after-w-icon').parent().removeAttr('disabled');
    }
    $('.week-list p label').html(getTimePeriod(currentWeek));
    getTotalData2('',startPeriod,endPeriod);
});
...
function setDate(y,m,d) {
    ...
    if(y>globalYear || (y==globalYear && m> globalMonth) ||(y==globalYear && m == globalMonth && d > globalDate)){
        ...
        $('.change-day button .after-icon').parent().attr("disabled",false); 
    }else if(y==globalYear && m == globalMonth && d == globalDate){
        $('.change-day button .after-icon').parent().attr("disabled","disabled");
    }
...
function getPunchData2(date) {
    ...
    $.each(listData,function(m,n){
        var amStardTime =addZero((new Date(n.start*1000)).getHours())+':'+addZero((new Date(n.start*1000)).getMinutes());
        var apEndTime = addZero((new Date(n.end*1000)).getHours())+':'+addZero((new Date(n.end*1000)).getMinutes());
        var updatedTime = addZero((new Date(n.updated_at*1000)).getHours())+':'+addZero((new Date(n.updated_at*1000)).getMinutes());
...
function getTotalData2(val1,val2,val3){
    ...
    $.each(listData,function(x,y){
        $('.ying').html(y.shouldBeSchedule+'节');
        $('.shi').html(y.actualSchedule+'节');
            ...
            if(n.name=='缺勤'){
               if(n.number){
                ...
                if(globalType == 2) {
                    ...
                    $.each(n.data,function(a,b){
                        var created_at = parseInt(b.created_at*1000);
                        //var mm = parseInt((new Date(created_at)).getMonth())+1;
                       // var ddd = (new Date(created_at)).getFullYear()+'/'+mm+'/'+(new Date(created_at)).getDate();

                        que += '<div><p><span>'+getLocalTime(b.created_at)+' </span><span></span></p><label></label></div>';
                        //que += '<div><p><span>'+ddd+' </span></p><label>4444</label></div>';
                    });
                }
                ...
                if(globalType == 2){
                    $.each(n.data,function(a,b){
                        var mm = parseInt((new Date(b.updated_at*1000)).getMonth())+1;
                        var ddd = (new Date(b.updated_at*1000)).getFullYear()+'-'+mm+'-'+(new Date(b.updated_at*1000)).getDate();
                        chi += '<div><p><span>'+ddd+' '+ getWeek((new Date(b.updated_at * 1000)).getFullYear(),mm-1,(new Date(b.updated_at * 1000)).getDate())+' </span><span>第'+b.lesson+'节 | '+ b.course_name+'课</span></p><label>迟到'+formatSeconds(b.time)+'分钟</label></div>';
                    });
                }else{ //学校模式
                    $.each(n.data,function(a,b){
                       // var mm = parseInt((new Date(b.time*1000)).getMonth())+1;
                       // var ddd = (new Date(b.time*1000)).getFullYear()+'-'+mm+'-'+(new Date(b.time*1000)).getDate();
                        chi += '<div><p><span>'+getLocalTime(b.created_at)+' </span></p><label>迟到'+ formatSeconds(b.time)+'</label></div>';
                    });
                }
                ...
                if(globalType == 2) {
                    $.each(n.data, function (a, b) {
                        ...
                        zao += '<div><p><span>' + ddd + ' ' + getWeek((new Date(b.updated_at * 1000)).getFullYear(),mm-1,(new Date(b.updated_at * 1000)).getDate()) + ' </span><span>第' + b.lesson + '节 | ' + b.course_name + '课</span></p><label>早退'+formatSeconds(b.time)+'</label></div>';
                    });
                }else{
                    $.each(n.data,function(a,b){
                       // var mm = parseInt((new Date(b.time*1000)).getMonth())+1;
                       // var ddd = (new Date(b.time*1000)).getFullYear()+'-'+mm+'-'+(new Date(b.time*1000)).getDate();
                        zao += '<div><p><span>'+getLocalTime(b.created_at)+' </span></p><label>早退'+ formatSeconds(b.time)+'</label></div>';
                    });
                }
...
//getTermList(flagKey);
var flagKey; //页面上显示的当前的
$.get(getApiUrl+'/student/attendance/semester?access_token='+sessionStorage.getItem('access_token'),function(data){
    $.each(data.data,function(k,v){

        semesterArr.push(v);
        var cTime = Date.parse(new Date());//当前时间转时间戳
        var bTime = Date.parse(new Date(v.start));//开始时间转时间戳
        var eTime = Date.parse(new Date(v.end));//结束时间转时间戳
        ...
        if(cTime >= bTime && cTime <= eTime){
            ...
            firstSTime = v.start;
            firstETime = v.end;
            $('.term-list .termKey').attr('data-key',k)
            $('.term-list .termKey').html(v.name);
        }
    });
});
...
$('.select-term-box').on('click','li',function(){
    if($(this).find('label').hasClass('checked-icon')){
        ...
        if(semesterArrKey == 0){
            $('.before-x-icon').parent().attr("disabled",'disabled');
        }
